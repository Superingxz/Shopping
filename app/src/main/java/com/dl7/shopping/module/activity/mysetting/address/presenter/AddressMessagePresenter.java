package com.dl7.shopping.module.activity.mysetting.address.presenter;

import com.dl7.shopping.module.activity.mysetting.address.baseview.IAddressMessageView;
import com.dl7.shopping.module.base.IBasePresenter;

/**
 * Created by Administrator on 2017/7/23.
 */

public class AddressMessagePresenter implements IBasePresenter {
    private final IAddressMessageView mView;

    public AddressMessagePresenter(IAddressMessageView mView) {
        this.mView = mView;
    }
}
